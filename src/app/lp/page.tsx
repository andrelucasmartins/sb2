

import { Button } from "@/components/ui/button";
import Link from "next/link";

interface LpProps {}

export default function Lp (props: LpProps) {
 return (
   <section className="flex h-lvh flex-col items-center justify-center space-y-4 bg-white text-gray-500">
     <svg
       xmlns="http://www.w3.org/2000/svg"
       width="142"
       height="161"
       viewBox="0 0 142 161"
     >
       <g fill="none">
         <path
           fill="#FFF5CD"
           d="M127.631 118.241h-9.463c12.491 0 12.491 18.88 0 18.88h-9.463c12.491 0 12.491 18.879 0 18.879H76.72c-17.98 0-28.673-4.343-43.72-9.44V89.922c11.45-5.758 17.318-7.836 24.51-15.01 7.475-7.457 12.112-26.808 13.153-37.38l.38-4.532 4.636.283c17.79 1.133 17.79 34.078 4.637 47.2h47.315c12.492 0 12.492 18.879 0 18.879 12.492 0 12.492 18.88 0 18.88z"
         ></path>
         <path
           fill="#4DBBEB"
           d="M62.204 16.315L57.5 6.857c-2.78-5.588 5.629-9.814 8.418-4.23l4.706 9.458c2.804 5.635-5.668 9.743-8.419 4.23zm28.234-4.23l4.705-9.458c2.767-5.561 11.208-1.377 8.419 4.23l-4.706 9.458c-2.793 5.598-11.217 1.395-8.418-4.23zm27.736 25.759h-9.41c-6.204 0-6.229-9.458 0-9.458h9.41c6.203 0 6.228 9.458 0 9.458zm-65.877 0h-9.41c-6.204 0-6.229-9.458 0-9.458h9.41c6.203 0 6.228 9.458 0 9.458z"
         ></path>
         <path
           fill="#FEDBAB"
           d="M118.586 137h-9.51c12.553 0 12.553 19 0 19H76.934c-18.068 0-28.814-4.37-43.934-9.5V118h85.586c12.552 0 12.552 19 0 19z"
         ></path>
         <g fill="#FEC478">
           <path d="M138.347 99.138c8.156-9.09 1.612-23.672-10.622-23.672H89.791c11.38-20.263.948-47.344-15.08-47.344h-3.888c-2.656 0-4.742 2.083-4.742 4.735 0 10.794-4.363 30.773-12.329 38.821-6.26 6.155-14.32 9.375-22.95 13.73-1.613.853-2.656 2.462-2.656 4.261v56.813c0 1.989 1.327 3.882 3.225 4.45 14.889 5.019 26.459 9.754 45.332 9.754h32.055c9.958 0 16.975-10.132 13.182-19.412 8.061-2.177 12.708-10.984 9.484-18.937 2.845-.758 5.216-2.273 6.923-4.261 4.552-5.113 5.027-13.256 0-18.938zM99.274 122.81h18.968c6.259 0 6.259 9.469 0 9.469H99.274c-6.259 0-6.259 9.469 0 9.469h9.484c6.259 0 6.259 9.468 0 9.468H76.703c-16.787 0-26.934-4.071-39.073-8.143V92.605c9.673-4.83 15.648-7.197 23.14-14.677 8.25-8.238 13.467-28.691 14.605-40.338 13.372.853 11.76 29.165 1.612 39.296-3.035 2.936-.949 8.049 3.32 8.049h47.418c6.26 0 6.26 9.468 0 9.468H99.274c-6.259 0-6.259 9.47 0 9.47h28.451c6.26 0 6.26 9.468 0 9.468H99.274c-3.13 0-4.742 2.367-4.742 4.735 0 2.367 1.613 4.734 4.742 4.734z"></path>
           <path d="M131.424 122.337c3.224 7.954-1.423 16.76-9.484 18.938 3.794 9.279-3.224 19.41-13.182 19.41H76.703c-18.873 0-30.443-4.734-45.332-9.752-1.897-.569-3.225-2.462-3.225-4.45v-28.407h9.484v24.998c12.139 4.071 22.286 8.143 39.073 8.143h32.055c6.259 0 6.259-9.469 0-9.469h-9.484c-6.259 0-6.259-9.469 0-9.469h18.968c6.259 0 6.259-9.469 0-9.469H99.274c-3.13 0-4.742-2.367-4.742-4.734h43.815c-1.707 1.989-4.078 3.503-6.923 4.261z"></path>
         </g>
         <path
           fill="#4DBBEB"
           d="M33.25 76H4.75C2.09 76 0 78.077 0 80.722v75.556C0 158.922 2.09 161 4.75 161h28.5c2.66 0 4.75-2.077 4.75-4.722V80.722C38 78.077 35.91 76 33.25 76z"
         ></path>
         <path
           fill="#4F8EB9"
           d="M38 118v38.222c0 2.676-2.09 4.778-4.75 4.778H4.75C2.09 161 0 158.898 0 156.222V118h38z"
         ></path>
       </g>
     </svg>

     <h1 className="text-center text-xl font-bold"> Tudo certo!</h1>
     <p className="text-xl">
       Clique em continuar para ser direcionado para a página solicitada.{" "}
     </p>
     <Button
       variant={"default"}
       size={"lg"}
       className="rounded-full bg-green-600 hover:bg-green-400"
     >
       <Link href="/">Continuar</Link>
     </Button>
   </section>
 );
};
